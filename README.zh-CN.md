...
需要连接的 `td` 指令会按需在后台启动 Daemon；`--help`、`--version` 与离线
`ops types` 类型目录不会启动服务。可直接将 Agent 拖入已打开的作品，无须重开
TouchDesigner。Agent 的 Connection 页面显示 `waiting_for_daemon`、`connecting`、
`online` 或明确的认证／注册错误。Windows 采用本机 `Win32_Process.Create` 启动，
使 Daemon 与明确打开的 TD 不受调用端终端 Job 生命周期牵制；后台启动不显示控制台窗口。
...
`project open` 返回操作系统 PID，不代表 Agent 已上线，也不关闭已有 Instance。
`install-skill` 将随版本内嵌于 `td-agent.exe` 的 `td-cli` skill 安装到
`$CODEX_HOME/skills/td-cli`（默认 `~/.codex/skills/td-cli`），无须另外下载。
目标目录已有内容时须明确加 `--replace`，可用 `--destination` 指定其他 skill 目录。
Skill 按任务链接 Derivative 理论与真实的 CLI help，避免猜类型、参数和数值语义。

保存使用已有的 live `project save <当前路径> --expected-sha256 <磁盘哈希>`；
成功结果包含磁盘 SHA-256，无须关闭 TD。磁盘前置条件失败如实返回
`project_file_changed`，不再误报 `protocol_incompatible`。保存超时保留 Request ID，
先查 outcome 与磁盘状态再决定下一个 mutation。可执行文件升级仍不替换作品内嵌的 Agent。

<!-- doc-section: license -->

## 授权

td-cli 自有代码采用 [MIT 许可](LICENSE)。
[第三方许可清单](THIRD_PARTY_NOTICES.md) 保留依赖组件的许可、版权声明与源代码链接。
四个 Release Artifact 与 Python 包均附带这些文件。TouchDesigner 仍受 Derivative 的许可约束。
...
`%LOCALAPPDATA%\touchdesigner-cli`。`td-daemon start` 会在背景程序就绪后返回，且不会留下
打开的控制台窗口。
...
Protocol v3 是唯一 runtime protocol，不提供 v1 alias 或 fallback。Request 依次经过
`queued`、`dispatched`、`accepted`、`running` 后才进入终态。Daemon 必须先持久化再派发，
每个 Instance 依 FIFO 仅允许一个已授权 Request，且每次重连都以新的 Connection ID 隔离。
...
Agent 在发出 `request_accepted` 前会先保留有界 outcome 容量，只执行符合 immutable execution
authorization 的工作，并保留所有 post-accept `succeeded`、`failed`、`unknown` outcome，直到
Daemon 完成记录。上限为 64 条、每条 canonical outcome 256 KiB、合计 16 MiB。Command、
超过锁定版 SocketIO DAT 单一事件实测范围的 outcome，会拆成依序且经过 identity 验证的 24 KiB
区块，重组完成后才提交公开结果。Command、heartbeat 与 drain timer 使用 TouchDesigner
...
目前已验证 canonical Agent 0.3.1 / 0.4.0 → 0.5.0、TouchDesigner 2025.32050；
相同 0.5.0 仅验证、不改文件，不代表任意历史或未来版本都受支持。
...
`project.save` 只保存当前已存在的本地 `.toe`。先用 `project metadata` 确认
路径、排除其他写入者，再取得磁盘文件摘要：
...
路径与 SHA-256 必须在执行前相符；这不是跨进程的原子锁。文件上限 64 MiB，
拒绝链接／reparse 路径。成功结果包含实际磁盘路径、字节数及 SHA-256。
不另存新文件名，也不保存外部 TOX。保存后无法确认结果时报告
`project_save_outcome_unknown`；先查 retained Request 与磁盘，不自动重做。

Protocol v3 在 SocketIO 边界以 JSON 文本保留 Command 的布尔、整数及 null 类型。
CLI、Daemon 与项目内嵌 Agent 必须一起升级；v2/v3 双向拒绝注册。
可编辑的 `StrMenu`（例如 Select CHOP 的 `channames`）接受任意字符串，
普通 `Menu` 仍只接受列出的选项名称。
